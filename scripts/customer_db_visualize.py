...
def query_recent_rows(conn: sqlite3.Connection, table: str, limit: int) -> tuple[list[str], list[sqlite3.Row]]:
    # Project-known tables (prefer "reasonable" reverse order).
...
def _print_help() -> None:
    print(
        "\n".join(
            [
                "用法：",
                "  - 用户列表（默认）：python3 scripts/customer_db_visualize.py [--db PATH] [--limit N]",
                "  - 指定用户对话：  python3 scripts/customer_db_visualize.py --user EXTERNAL_USERID [--open-kfid OKFID] [--db PATH]",
                "  - 昵称检索用户：  python3 scripts/customer_db_visualize.py --nickname 关键字 [--open-kfid OKFID] [--db PATH]",
...
def list_chatted_users(conn: sqlite3.Connection, limit: int) -> tuple[list[str], list[list[Any]]]:
    """
    Output: open_kfid / external_userid / nickname / last_time
    nickname comes from customer_profiles; if missing, use '-'
    """
...
def search_users_by_nickname(
    ...
    *,
    ...
    """
    Fuzzy search users by customer_profiles.nickname.
    last_time comes from conversation_messages.
    """
...
def main() -> int:
    ...
    # Default: read project_root/.env (not scripts/.env)
    ...
    try:
        # Default mode: user list / one user's conversation
        if not schema_mode:
            if user_id:
            ...
            if nickname_kw:
                ...
                _print_kv(
                    "meta",
                    {"db": db_path, "now": _fmt_ts(_now_ts()), "nickname": nickname_kw, "limit": limit, "open_kfid": open_kfid or "-"},
                )
        ...
        for t in tables:
            ...
            for c in cols:
                col_rows.append([c["cid"], c["name"], c["type"], "Y" if int(c["notnull"] or 0) else "", "Y" if int(c["pk"] or 0) else "", _clip(c["default"], 32)])
            print("字段：")
